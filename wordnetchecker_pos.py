...
for w in words:
    parts_of_speech = [wn.ADJ, wn.NOUN, wn.ADV, wn.VERB, wn.ADJ_SAT]
    for pos in parts_of_speech:
        syns = [i for i in wn.synsets(w) if str(i.pos()) == pos]
        hypos = []
        hypers = []
        for a in syns:
...
allstotal = hypostotal + hyperstotal + synstotal + holototal + merostotal

for pos in parts_of_speech:
    print "{} {}".format(pos, np.mean(np.array([p[0] for p in allstotal if p[1] == pos])))
