...
def get_synsets(word, pos):
    syns = [i for i in wn.synsets(word) if str(i.pos()) == pos]
    return syns
...
def synononymous(worda, wordb, pos):
    ...
    return math.ceil(jaccard(syns_a, syns_b))


def get_all_possible_hyponyms(a, b, pos):
    ...
    return len(hyponyms) * len(synsets_b)


def get_all_possible_hypernyms(a, b, pos):
    synsets_a = wn.synsets(a, pos)
    ...
    synsets_b = wn.synsets(b, pos)
    return len(hypernyms) * len(synsets_b)


def get_all_possible_meronyms(a, b, pos):
    syn_a = wn.synsets(a, pos)
    syn_b = get_meronyms(wn.synsets(b, pos))
    return len(syn_a) * len(syn_b)


def get_all_possible_holonyms(a, b, pos):
    syn_a = wn.synsets(a, pos)
...
def hyoponomous(a, b, pos):
    ...
    synsets_b = wn.synsets(b, pos)
    return math.ceil(jaccard(hyponyms, synsets_b))
...
def hypernomous(a, b, pos):
    ...
    synsets_b = wn.synsets(b, pos)
    return math.ceil(jaccard(hypernyms, synsets_b))


def meronymous(a, b, pos):
    syn_a = wn.synsets(a, pos)
    syn_b = get_meronyms(wn.synsets(b, pos))
    return math.ceil(jaccard(syn_a, syn_b))
